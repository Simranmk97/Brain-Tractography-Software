...
for i = 1:length(break_index_Y)
    ...
    if k(i) == k(i) % this was supposed undo the overwriting of the last row of the previous pathway in the new pathway's list. This only worked for the RID.tract(1) and RID.tract(2), but not for the rest. I think break_index_Y nad Y1 indexes are not aligning right.
        k(i) = k(i) + 1;
